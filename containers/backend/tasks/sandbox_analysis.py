...
def sandbox_analysis_task(self, scan_id: str):
    ...
    # \u2500\u2500 V8/V10: Write sandbox.html for DOM cloaking detection \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    # consistency.py checks os.path.exists("sandbox.html") and marks the DOM
    # channel indeterminate when it is absent. Writing the sandbox DOM here
    # enables the full browser-DOM vs sandbox-DOM cloaking comparison.
    # Sandbox may capture the page source as:
    #   - sandbox_result["dom"]["page_source"]  (preferred, Playwright outerHTML)
    #   - sandbox_result["dom_content"]          (legacy flat key)
    # If neither key is present the sandbox didn't capture a DOM (e.g. it
    # crashed or timed out) — we skip silently and consistency.py degrades
    # gracefully as before.
    _sandbox_html = (
        (sandbox_result.get("dom") or {}).get("page_source")
        or sandbox_result.get("dom_content")
        or ""
    )
    if _sandbox_html:
        _sandbox_html_path = os.path.join(scan_dir, "sandbox.html")
        _sandbox_html_tmp = _sandbox_html_path + ".tmp"
        try:
            with open(_sandbox_html_tmp, "w", encoding="utf-8", errors="replace") as _f:
                _f.write(_sandbox_html)
            os.replace(_sandbox_html_tmp, _sandbox_html_path)
            logger.info(
                "[%s] sandbox.html written (%d chars)", scan_id, len(_sandbox_html)
            )
        except Exception:
            logger.exception("[%s] Failed to write sandbox.html — DOM comparison will be indeterminate", scan_id)
    else:
        logger.debug(
            "[%s] No DOM page_source in sandbox result — sandbox.html not written; "
            "DOM consistency comparison will be indeterminate", scan_id
        )
